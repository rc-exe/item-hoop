...
('Electronics', 'Phones, laptops, gadgets and electronic devices', 'Smartphone'),
('Books', 'Textbooks, novels, educational materials', 'Book'),
('Sports', 'Sports equipment, fitness gear, outdoor activities', 'Dumbbell'),
('Music', 'Musical instruments, audio equipment, records', 'Music'),
('Fashion', 'Clothing, accessories, shoes', 'Shirt'),
('Home & Garden', 'Furniture, decor, gardening tools', 'Home'),
('Gaming', 'Video games, consoles, gaming accessories', 'Gamepad2'),
('Art & Crafts', 'Art supplies, handmade items, craft materials', 'Palette'),
('Automotive', 'Car parts, accessories, tools', 'Car'),
('Other', 'Items that don\'t fit other categories', 'Package');

-- Function to update user ratings after exchange completion
CREATE OR REPLACE FUNCTION public.update_user_rating_stats()
RETURNS TRIGGER AS $$
BEGIN
  -- Update the rated user's rating and total exchanges
  UPDATE public.profiles 
  SET 
    rating = (
      SELECT COALESCE(AVG(rating), 0) 
      FROM public.exchange_ratings 
...
CREATE TRIGGER update_user_rating_on_new_rating
  AFTER INSERT ON public.exchange_ratings
  FOR EACH ROW
